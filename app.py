...
def preprocess(X, min_ngram=1, max_ngram=2, vec='Tf-Idf', test=False, vocab=None):
    ...
    X = X.apply(lambda x: ' '.join([ps.stem(word) for word in x if word not in Stopwords and word in english_words_set and word not in common_words and word is not ' ']))
